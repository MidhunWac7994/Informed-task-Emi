...
const useInstallmentLogic = () => {
  ...
  const installmentAmount = recommendedAmount ? parseFloat(recommendedAmount) / installmentCount : 0;

  useEffect(() => {
    const newDueDates = {};
    const startDate = new Date();
    startDate.setDate(startDate.getDate() + 1); // Set the first due date to tomorrow

    for (let i = 1; i <= installmentCount; i++) {
      const newDate = new Date(startDate);
      newDate.setMonth(newDate.getMonth() + i - 1); // Add months for each installment
  ...
  const handleInstallmentSelect = (installNo) => {
    setSelectedInstallments((prev) =>
      prev.includes(installNo) ? prev.filter((num) => num !== installNo) : [...prev, installNo]
    );
  };
